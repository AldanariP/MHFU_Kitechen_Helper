...
class Model:
    ...
    def getBonus(
        ...
        showNegativeBonuses: bool = False,
    ) -> list[Bonus]:
        ...
        if not showNoEffect:  # if show effect is True -> filter No Effect
            resultBonuses = [
                ...
                if not bonus.isOfBuffType(BuffType.NOEFFECT)
            ]

        if not showNegativeBonuses:
            resultBonuses = [
                bonus for bonus in resultBonuses if not bonus.is_negative()
            ]
                ...
                if ingredientType in IngredientType:
                    ...
                    for ingredientName in chef_number_data[ingredientType]:
                        self.ingredientList.append(
                            Ingredient(ingredientName, chefNumber, ingredientType)
                        )
                else:
                    # Bonuses
